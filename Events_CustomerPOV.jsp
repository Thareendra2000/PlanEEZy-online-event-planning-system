...
<%
//get log in email from the session
...
<%-- <sql:query var="events" dataSource="${dataSource}">select* from event where CusID = 1000;</sql:query> --%>
<main id="main" class="main">
<p class="card-text"><a href="EventCreate.jsp" class="btn btn-warning">Add an Event </a></p>
...
<!--       this is for update function -->
            <c:url value="EventUpdate.jsp" var="eventupdate">
				<c:param name="eventID" value="${row.eventID}"/>
				<c:param name="Date" value="${row.Date}"/>
				<c:param name="Type" value="${row.Type}"/>
				<c:param name="Noguest" value="${row.Noguest}"/>
			</c:url>
            <p class="card-text"><a href="${eventupdate}" class="btn btn-success">Update <i class="bi bi-pen"></i></a></p>
<!--       this is for delete function -->
			<c:url value="EventDelete.jsp" var="eventdelete">
					<c:param name="eventID" value="${row.eventID}"/>
					<c:param name="Date" value="${row.Date}"/>
					<c:param name="Type" value="${row.Type}"/>
					<c:param name="Noguest" value="${row.Noguest}"/>		
			</c:url>
             <p class="card-text"><a href="${eventdelete}" class="btn btn-danger">Delete <i class="bi bi-trash3"></i></a></p>
